      ...
      <div className="flex w-full flex-col justify-between lg:w-1/2">
        ...
        <div>
          {TimelineItems.map((item, index) => (
            ...
            >
              <h1
                className={`cursor-pointer pb-2 text-xl font-light md:text-2xl ${activeIndex === index ? 'text-blaxk' : 'text-gray-400'}`}
                onClick={() => setActiveIndex(index)}
              >
                {item.title}
              </h1>
              <p className="text-gray pb-2 text-sm">{item.description}</p>
            </div>
          ))}
